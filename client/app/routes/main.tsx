        ...
        {candidates.map((data, index) => (
          <AccordionItem value={`item-${index}`}>
            ...
            <AccordionContent>
              ...
              <div className="text-zinc-400 mt-2">How would you benefit the club?</div>
              <div>{data.benefitToClub}</div>
              <div className="text-zinc-400 mt-2">Are there any initiatives or events you would like to run if you were elected?</div>
              <div>{data.initiativesOrEvents}</div>
              <div className="text-zinc-400 mt-2">Are you currently a part of any other club's committee?</div>
              <div>{data.currentCommittee}</div>
              <div className="text-zinc-400 mt-2">Have you previously been part of any other club's committee?</div>
              <div>{data.previousCommittee}</div>
              <div className="text-zinc-400 mt-2">Can you attend the AGM?</div>
              <div>{data.canAttendAGM}</div>
              <div className="text-zinc-400 mt-2">If no, what should be said on your behalf at the AGM?</div>
              <div>{data.messageForAGM}</div>
            </AccordionContent>
          </AccordionItem>
        ))}
      </Accordion>
      <StatusBar status={Status.Open} position="President" candidates={candidates} />
    </main>
  );
}
