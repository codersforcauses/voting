...
export function getVotePreference(this: VotingObject, vote_id: number, candidate_id: number) {
  return this.db.select().from(votePreferencesTable).where(
    and(
...
export function getVotePreferencesForVote(this: VotingObject, vote_id: number) {
  return this.db.select().from(votePreferencesTable).where(eq(votePreferencesTable.vote_id, vote_id));
}
...
export function updateVotePreference(
  ...
  return this.db.update(votePreferencesTable).set(data).where(
    and(
      eq(votePreferencesTable.vote_id, vote_id),
      eq(votePreferencesTable.candidate_id, candidate_id)
    )
...
export function deleteVotePreference(this: VotingObject, vote_id: number, candidate_id: number,) {
  return this.db.delete(votePreferencesTable).where(and(
    eq(votePreferencesTable.vote_id, vote_id),
    eq(votePreferencesTable.candidate_id, candidate_id)
  )).returning();
}
