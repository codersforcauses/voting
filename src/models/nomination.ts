...
export function getNominationsForPosition(this: VotingObject, position_id: number) {
  return this.db.select().from(nominationsTable)
...
export function deleteNomination(this: VotingObject, candidate_id: number, position_id: number) {
  return this.db.delete(nominationsTable)
    .where(and(
