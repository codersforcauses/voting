...
import type { Candidate, Count, Seat, TransferValue } from "./types";
...
export default class HareClark extends Race {
  ...
  count(): TransferValue[] {
    ...
    while (elected.length < this.openings) {
      ...
      const buffer = this.findElectedCandidates(count, quota);

      // Do we have enough positions?
      // This is our chance to early exit (and maybe break ties)
      const total = elected.length + buffer.length;
      ...
      if (elected.length + this.candidates.size == this.openings) {
        const remaining = []
        for (const [c, votes] of this.candidates.entries()) {
          remaining.push({
            candidate: c,
            count: -1,
            tv: 0,
